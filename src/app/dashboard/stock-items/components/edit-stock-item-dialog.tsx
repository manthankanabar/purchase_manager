...
export function EditStockItemDialog({
  ...
  const [categoryOptions, setCategoryOptions] = useState<{ id: string; name: string; prefixCode: string }[]>([]);
  const [groupOptions, setGroupOptions] = useState<{ id: string; name: string; code: string; categoryId: string }[]>([]);
  const [filteredGroups, setFilteredGroups] = useState<{ id: string; name: string; code: string; categoryId: string }[]>([]);

  // Define a more specific form schema for the component
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      cgstRate: stockItem.cgstRate,
      sgstRate: stockItem.sgstRate,
      igstRate: stockItem.igstRate,
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        // Fetch categories
        const categoriesResponse = await axios.get("/api/stock-categories");
        setCategoryOptions(categoriesResponse.data);

        // Fetch groups
        const groupsResponse = await axios.get("/api/stock-groups");
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      setLoading(true);
      await axios.put(`/api/stock-items/${stockItem.id}`, data);
      toast.success("Stock item updated successfully");
      onOpenChangeAction(false);
      router.refresh();
    } catch (error: any) {
      toast.error(error.response?.data || "Failed to update stock item");
    } finally {
      setLoading(false);
    }
  };
